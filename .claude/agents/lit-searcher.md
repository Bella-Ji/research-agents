...
확장된 검색어 각각에 대해 grep으로 매칭 파일을 수집한다.
점수 계산 없이 Grep 매칭 결과를 그대로 쓴다.
...
각 매칭 파일의 핵심 필드를 확인한다:
```bash
grep -m 8 "^title:\|^tags:\|^source_folder:\|^author:\|^year:\|^journal:" 파일경로
```

매칭 파일 전체를 목록으로 나열한다. 15개 초과 시 처음 15개를 사용한다.
관련도 판단은 점수 계산 없이 한줄 요약·title·tags 내용을 읽어서 정성적으로 한다.

**종료조건**: Grep 매칭은 최대 3회까지만 시도하고, 매칭 파일이 모이면 즉시 합성으로 넘어간다. 같은 의미의 검색어 변형을 반복하지 않는다.
...
1. **저자 (연도)** — *제목* [`파일명_요약.md`]
   - 관련 이유: 한 문장 설명 (한줄 요약·title·tags 내용 기반 정성 판단)
